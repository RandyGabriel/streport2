...
foreach ($branches as $branch_name) {
    ...
    if ($filter == 'tipe') {
        ...
        FROM databasemaju
        WHERE branch_name = '$branch_name' AND brand = 'REALME SMARTPHONE'
        ...
        if ($result->num_rows > 0) {
            ...
            // Pastikan semua product_name muncul pada bulan terakhir meskipun qty 0
            foreach ($data as $product_name => $qty_per_month) {
                if ($qty_per_month['December'] == 0) {
                    $latest_month_qty[$product_name] = 0;
                }
            }
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                // Simpan qty bulan terakhir
                if ($bulan == 'December') {
                    $latest_month_qty[$marketing] = $data[$marketing][$bulan];
                }
            }

            // Pastikan semua marketing muncul pada bulan terakhir meskipun qty 0
            foreach ($data as $marketing => $qty_per_month) {
                if ($qty_per_month['December'] == 0) {
                    $latest_month_qty[$marketing] = 0;
                }
            }
            ...
            echo "<table>
                    ...
                    foreach ($latest_month_qty as $marketing => $qty_last_month) {
                        echo "<tr>
                                <td class='left-align'>$marketing</td>";
                        foreach ($bulan_array as $bulan) {
                            $cell_class = $data[$marketing][$bulan] == 0 ? 'class="zero-value"' : '';
                            echo "<td $cell_class>" . number_format($data[$marketing][$bulan], 0, ',', '.') . "</td>";
                            $total_qty_footer[$bulan] += $data[$marketing][$bulan]; // Tambahkan total qty untuk footer hanya dari baris yang muncul
                        }
                        echo "</tr>";
                    }
                ...
                if ($result->num_rows > 0) {
                    ...
                    // Pastikan semua alias muncul pada bulan terakhir meskipun qty 0
                    foreach ($data as $alias => $qty_per_month) {
                        if ($qty_per_month['December'] == 0) {
                            $latest_month_qty[$alias] = 0;
                        }
                    }
